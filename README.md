...
| **generate_html**   | create an interactive static HTML website for exploring netcdf files                        | netcdf_explorer_env        |
 ...
 | **custom_processor** | run a custom processor to convert/filter data files                                       | customisable               |
...
### embedding the eo_pipelines version in netcdf4 output files

Use the `eo_pipelines:rubber_stamp` node to add the pipeline YAML content, path and eo_pipelines versioning information into the output file
...
Non-trivial updates to a pipeline stage should trigger version increments for both the stage and the eo_pipelines package.
